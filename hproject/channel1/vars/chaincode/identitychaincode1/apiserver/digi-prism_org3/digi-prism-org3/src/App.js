...
function App() {
 ...
 {/* ------------------------------------------------------------------  
  ...
  fetch(`/api/readMarblePrivateDetails/marble${marbleName}`)
    .then((res) => {
      if (!res.ok) {
        throw new Error('User does not have read access permission on privatedata in chaincodeName');
      }
      return res.json();
    })
    .then((data) => {
    ...
    .catch((error) => {
      ...
      if (error.message.includes ('Failed to get private details')) {
        // Display error message for permission issue
        setreadMarblePrivateDetailsData('You do not have permission to access the private data');
      } else {
        // Display general error message for other issues
        setreadMarblePrivateDetailsData('Failed to fetch data');
      }
    });
};
...
const getreadMarblePrivateDetails = () => {
  // Display error message directly without making a backend request
  setreadMarblePrivateDetailsData('User does not have permission to access private data');
};
  ...
  {readMarblePrivateDetailsDataVisible && readMarblePrivateDetailsdata && (
    <div style={{ display: 'flex', justifyContent: 'center', marginTop: '10px' }}>
      <div style={{ maxWidth: '500px', overflowWrap: 'break-word', backgroundColor: 'white', color: 'black', padding: '10px', border: 'none', borderRadius: '5px', fontSize: '0.5em' }}>
        ...
        <strong>Error:</strong> {readMarblePrivateDetailsdata}
        </div>
   </div>
  )}
</div>
